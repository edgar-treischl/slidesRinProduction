...
## .white[`r emoji::emoji("one")` Get Rid of Bad Habits ] 



---
## 01 Develop: Start with a clean slate 

.pull-left[

- Based on Bryan et al. 2021: Start with a clean slate and restart R: Do not save the workspace and do not load the workspace from an `.Rdata` file. 
...
## 01 Develop: Fix paths

Abandon absolute paths, they will break anyway `r emoji::emoji("skull")`. Fix paths with the `here` package:
...
Code lives in .R files, Code and Text in .Rmd If you need to create a document from a source code, run:
...
## 02 Build: Github Actions and usethis
...
# .white[`r emoji::emoji("spider")` Don't forget to test] 
...
## .red[`r emoji::emoji("four")` Document it] 
